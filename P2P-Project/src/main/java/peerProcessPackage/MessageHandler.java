 ...
 */
    ...
    public PeerMessage handleActualMsgFromPeer(PeerMessage msg) {
        switch (msg.getType()) {
            ...
            case Piece: { // operations to do when "PIECE" is received 
                ...
                ftupdateLogger.downloadedPieceLog(otherPeerId, piece.getPieceIndex(), fileManager.getNumberOfReceivedParts());
                return requestPiece();
            }
        }
    ...
    private PeerMessage requestPiece() {
        if (!isChokedByOtherPeer) {
            int pieceIndex = fileManager.determinePieceToBeRequested(peerManager.getReceivedParts(otherPeerId));
            if (pieceIndex >= 0) {
                LoggerConfiguration.getLogger().info("Request for piece with index :" + pieceIndex + "being requested " + " from " + otherPeerId);
                return new RequestMessage (pieceIndex);
            }
            else {
