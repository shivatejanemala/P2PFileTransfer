...
public class RequestedParts {
    ...
    /**
     * Function to identify the piece index to be requested
     * 
     */
    synchronized int getPartToRequest(BitSet bitset) {
        bitset.andNot(requestedParts);
        if (!bitset.isEmpty()) {
            final int pieceIndex = getRandomIndex(bitset);
