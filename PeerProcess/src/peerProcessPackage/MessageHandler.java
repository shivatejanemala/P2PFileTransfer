...
public class MessageHandler implements Runnable 
{
	...
	private static String currentPeer = null;
	public static int peerState = -1;
	RandomAccessFile raf;
	PeerProcess peerProcess;
	static Logger log = Logger.getLogger(MessageHandler.class.getName());
	
	// constructor
	public MessageHandler(String currentPeer)
	{
	...
	{
		log.info("Message Processor : msgType = "+ dataType + " State = "+state);
	}

	public void run()
	{
		DataMessage d;
		DataMessageDetails dataWrapper;
		String msgType;
		String rPeerId;
				
		while(running)
		{
			dataWrapper  = peerProcess.retrieveMsgFromQueue();
			...
			if(msgType.equals(DATA_MSG_HAVE) && state != 14)
			{
				...
				log.info(peerProcess.peerId + " receieved HAVE message from Peer " + rPeerId); 
			...
			else
			{
			 ...
			 case 2:
			   if (msgType.equals(DataMessageConstants.BITFIELD_MESSAGE_TYPE)) {
		 		  log.info(peerProcess.peerId + " receieved a BITFIELD message from Peer " + rPeerId);
